...
Kando will ✅ **validate** the configuration files. If you [launched Kando from a terminal](../../advanced-usage/launch-from-cli), it will print errors to the console if it finds any. In this case, the configuration will not be reloaded. If a configuration file is invalid at startup, Kando will refuse to start.
</Aside>
...
`soundTheme: "none"` <Badge text="soon!" variant="tip" /> | The directory name of the sound theme to use. Kando will first look for a directory with this name in the `sound-themes` subdirectory of the config directory. If it does not find one, it will look for a directory with this name in `.webpack/renderer/assets/sound-themes` in Kando's installation directory.
`soundVolume: 0.5` <Badge text="soon!" variant="tip" /> | The volume of the sound effects. This should be a value between 0 and 1.
...
[`"submenu"`](/getting-started/item-type/submenu),
[`"command"`](/getting-started/item-type/run-command),
[`"uri"`](/getting-started/item-type/open-uri),
[`"hotkey"`](/getting-started/item-type/simulate-hotkey),
[`"macro"`](/getting-started/item-type/simulate-macro),
[`"text"`](/getting-started/item-type/paste-text).
