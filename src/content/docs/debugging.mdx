...
## <Icon name="solar:document-text-bold-duotone" class="inline-icon" /> Specifying a Custom Configuration Directory
It may be beneficial to specify a custom configuration directory when developing Kando,
this way your main configuration files are not affected by your development changes.

As such, you can specify a custom configuration directory using the <a href="./launch-from-cli.mdx">`--config-dir`</a> flag when running Kando.

<Tabs syncKey="ide">
  <TabItem label="Zed" value="zed" icon="zed">
    ```json5 title=".zed/tasks.json"
    [
      {
        "label": "Kando: Main Process",
