...
```
#kando-menu
├ .menu-node.level-0.top.type-root.parent                       <-- This is the root item of the menu.
│  ├ .menu-node.level-1.top.type-submenu.grandchild                 It has three children, the third one
│  │  ├ .menu-node.level-2.right.type-button                        is a submenu which is currently open.
│  │  └ .menu-node.level-2.left.type-button
│  ├ .menu-node.level-1.right.type-submenu.grandchild
│  │  ├ .menu-node.level-2.top.type-button
│  │  └ .menu-node.level-2.bottom.type-button
│  └ .menu-node.level-1.left.type-submenu.active                <-- This is the currently active menu item.
│     ├ .menu-node.level-2.child.top.type-button                    It has four children.
│     ├ .menu-node.level-2.type-submenu.child.hovered.dragged   <-- The second child is a submenu which is
│     │  ├ .menu-node.level-3.grandchild.top.type-button            currently dragged around in marking or
│     │  └ .menu-node.level-3.grandchild.bottom.type-button         turbo mode.
│     ├ .menu-node.level-2.child.left.type-submenu
│     │  ├ .menu-node.level-3.grandchild.top.type-button
│     │  └ .menu-node.level-3.grandchild.bottom.type-button
│     └ .menu-node.level-2.child.right.type-button
├ .selection-wedges                                             <-- This is the layer which draws the
│                                                                   currently highlighted wedge background.
├ .wedge-separators                                             <-- This contains individual divs for the
│  ├ .separator                                                     separator lines between the wedges.
│  ├ .separator
│  └ .separator
...
Furthermore, the **menu node classes contain the type of the item** (`.type-root`, `.type-submenu`, or `.type-button`).
...
### CSS Properties of the Menu Elements
...
<details>
<summary>`--start-angle` and `--end-angle`</summary>
_Available on all elements with the `.menu-node` class except for the root node._

The angles at which the item's wedge starts and ends in degrees.
Can be useful for styling the background of the wedge.
</details>
...
<details>
<summary>`--parent-start-angle` and `--parent-end-angle`</summary>
_Available on all elements with the `.menu-node.parent` classes._

If the item is currently a parent of the active menu item, these properties contain the angles at which the parent's wedge starts and ends relative to the active menu item.
This is very similar to the `--start-angle` and `--end-angle` properties of the children of the active menu item, but for the back-navigation.
</details>
...
#### Selection Wedge Properties

The `.selection-wedges` element is a full-screen div which is drawn below the menu items.
It has four custom CSS properties which can be used to highlight the currently hovered wedge.

<details>
<summary>`--center-x` and `--center-y`</summary>
The coordinates of the center of the selection wedge in pixels.
</details>

<details>
<summary>`--start-angle` and `--end-angle`</summary>
The angles at which the currently hovered wedge starts and ends in degrees.
</details>

#### Wedge Separator Properties

The `.wedge-separators` contains individual divs for each separator line between the wedges.
Each separator is already rotated and positioned correctly, so you can just style them with a background color and height.
Per default, they have a width of `100vmax` and a height of `1px` which makes them span the whole screen.

<details>
<summary>`--angle`</summary>
The angle of the wedge in degrees, starting at the top with 0° and going clockwise.
</details>
...
.menu-node {
  ...
  /* If an item is clicked, we scale it down a bit. */
