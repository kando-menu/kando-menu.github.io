...
`hoverModeNeedsConfirmation: false` <Badge text="soon!" variant="tip" /> | If enabled, selecting final items in hover mode will require a confirmation click.
...
`hoverMode: false` <Badge text="soon!" variant="tip" /> | If enabled, menu items can be selected by hovering over them.
