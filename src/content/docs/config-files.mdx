...
`ignoreWriteProtectedConfigFiles: false` <Badge text="soon!" variant="success" /> | If set to `true`, Kando will not show a warning if the configuration files are write-protected.
`trayIconFlavor: 'color'` <Badge text="soon!" variant="success" /> | Can be one of `'color'`, `'light'`, `'dark'`, or `'none'`. The light, dark, and color flavors are only available on Windows and Linux. On macOS, the color always depends on your system colors. If set to `'none'`, the tray icon will be hidden.
